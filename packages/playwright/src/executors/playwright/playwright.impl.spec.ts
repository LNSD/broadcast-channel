...
import cypressExecutor, { PlaywrightExecutorOptions } from './playwright.impl';

jest.mock('@nrwl/devkit');
let devkit = require('@nrwl/devkit');

jest.mock('../../utils/playwright-version');
import { installedPlaywrightVersion } from '../../utils/playwright-version';
...
describe('Cypress builder', () => {
  ...
  const cypressOptions: PlaywrightExecutorOptions = {
    playwrightConfig: 'apps/my-app-e2e/playwright.json',
    parallel: false,
  ...
  let mockedInstalledCypressVersion: jest.Mock<
    ReturnType<typeof installedPlaywrightVersion>
  > = installedPlaywrightVersion as any;
  ...
  it('should call `Cypress.run` if headless mode is `true`', async () => {
    ...
    expect(cypressRun).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        project: path.dirname(cypressOptions.playwrightConfig),
      })
    );
  ...
  it('should call `Cypress.run` with provided cypressConfig as project and configFile', async () => {
    const { success } = await cypressExecutor(
      {
        ...cypressOptions,
        playwrightConfig: 'some/project/my-playwright.json',
      },
      mockContext
    );
    expect(success).toEqual(true);
